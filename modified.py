...
model = load_model('Capstone/models/mode_1648003085.h5')
...
while cap.isOpened:
  ...
  if cmdmode == 0:
    if len(action_seq) < 30:
      continue
    if len(set(action_seq[-30:-1])) == 1 and action_seq[-1] != None and action_seq[-1] != 'cmd':
    ...
    if len(cmd_seq) < 30:
      continue
    if len(set(cmd_seq[-30:-1])) == 1 and cmd_seq[-1] == 'cmd':
